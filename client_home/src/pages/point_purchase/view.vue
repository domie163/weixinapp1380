          ...
          <view class="">
            <uni-forms :modelValue="form"  v-if="is_view()">
              ...
              <uni-forms-item v-if="$check_field('get','user') || ($check_field('add','user') || $check_field('set','user'))" label="用户" name="user">
                  ...
                  v-if="user_group === '管理员' || (form['point_purchase_id'] && $check_field('set','user')) || (!form['point_purchase_id'] && $check_field('add','user'))"
                ></uni-data-select>
                <uni-data-select
                  v-model="form['user']"
                  :localdata="list_user_user"
              ...
              <uni-forms-item v-if="$check_field('get','user_name') || ($check_field('add','user_name') || $check_field('set','user_name'))" label="用户姓名" name="user_name">
                            <uni-easyinput type="text" v-model="form['user_name']" v-if="user_group === '管理员' || (form['point_purchase_id'] && $check_field('set','user_name')) || (!form['point_purchase_id'] && $check_field('add','user_name'))" :disabled="disabledObj['user_name_isDisabled']" />
                <!-- 仅查看 -->
              ...
              <uni-forms-item v-if="$check_field('get','credit_card_number') || ($check_field('add','credit_card_number') || $check_field('set','credit_card_number'))" label="积分卡号" name="credit_card_number">
                            <uni-easyinput type="text" v-model="form['credit_card_number']" v-if="user_group === '管理员' || (form['point_purchase_id'] && $check_field('set','credit_card_number')) || (!form['point_purchase_id'] && $check_field('add','credit_card_number'))" :disabled="disabledObj['credit_card_number_isDisabled']" />
                <!-- 仅查看 -->
                <text v-else-if="$check_field('get','credit_card_number')">
                  {{ form['credit_card_number'] }}
                </text>
                          </uni-forms-item>
              <uni-forms-item v-if="$check_field('get','purchase_points') || ($check_field('add','purchase_points') || $check_field('set','purchase_points'))" label="购买积分" name="purchase_points">
                            <uni-easyinput type="text" v-model="form['purchase_points']" v-if="user_group === '管理员' || (form['point_purchase_id'] && $check_field('set','purchase_points')) || (!form['point_purchase_id'] && $check_field('add','purchase_points'))" :disabled="disabledObj['purchase_points_isDisabled']" />
                <!-- 仅查看 -->
                <text v-else-if="$check_field('get','purchase_points')">
                  {{ form['purchase_points'] }}
                </text>
                          </uni-forms-item>
              <uni-forms-item v-if="$check_field('get','purchase_instructions') || ($check_field('add','purchase_instructions') || $check_field('set','purchase_instructions'))" label="购买说明" name="purchase_instructions">
                    <uni-easyinput type="textarea" v-model="form['purchase_instructions']" v-if="user_group === '管理员' || (form['point_purchase_id'] && $check_field('set','purchase_instructions')) || (!form['point_purchase_id'] && $check_field('add','purchase_instructions'))" :disabled="disabledObj['purchase_instructions_isDisabled']" />
                <!-- 仅查看 -->
                <text v-else-if="$check_field('get','purchase_instructions')">
                  {{ form['purchase_instructions'] }}
                </text>
                  </uni-forms-item>
      ...
      uni.chooseFile({
        ...
        success: function(res) {
          ...
          const uploadTask = uni.uploadFile({
            url: _self.$fullUrl('/api/point_purchase/upload?'),
